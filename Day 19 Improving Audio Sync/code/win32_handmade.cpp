...
internal void Win32DebugDrawVertical(win32_offscreen_buffer *TheGlobalBackBuffer, int X, int Top, int Bottom, uint32 Color){ 
	uint8 *Pixel = ((uint8 *)TheGlobalBackBuffer->Memory + X*TheGlobalBackBuffer->BytesPerPixel + Top*TheGlobalBackBuffer->Pitch);
	for(int Y = Top; Y < Bottom; ++Y){
		*(uint32 *)Pixel = Color;
		Pixel += TheGlobalBackBuffer->Pitch;
	}
}
