						...
						if(IsDown){
							if(Win32State->InputPlayingIndex == 0) {
								if(Win32State->InputRecordingIndex == 0) {
									...
									Win32EndRecordingInput(Win32State);
									Win32BeginInputPlayBack(Win32State, Win32State->InputPlayingIndex);
								}
							}
							else {
								Win32EndInputPlayBack(Win32State);
	...
	{ 	
		...
		if(RegisterClassA(&WindowClass))
		{
			...
			if(TheWindow){
				...
				HDC RefreshDC = GetDC(TheWindow);
				int Win32VRefreshRate = GetDeviceCaps(RefreshDC, VREFRESH);
				if(Win32VRefreshRate > 1){
					MonitorRefreshHz = Win32VRefreshRate;
				}
				real32 GameUpdateHz = (MonitorRefreshHz / 2.0f);
				real32 TargetSecondsPerFrame = 1.0f / (real32)GameUpdateHz;
				ReleaseDC(TheWindow, RefreshDC);
				...
				if(Samples && GameMemory.PermanentStorage && GameMemory.TransientStorage){
					...
					while(Running){
						...
						*NewKeyboardController = {};
						...
						//Missing if(!GlobalPause)
							
						POINT MouseP;
						GetCursorPos(&MouseP);
						ScreenToClient(TheWindow, &MouseP); //Maps from OS Desktop coordinated system to Game window coordinate system
						...
						*/
						if(Game.GameGetUpdateRenderer){
							Game.GameGetUpdateRenderer(&Thread, &GameMemory, NewInput, &Buffer);
						}
						...
						if((GlobalSecondaryBuffer->GetCurrentPosition(&PlayCursor, &WriteCursor)) == DS_OK) { 
							...
							*/
							if(Game.GameGetSoundSamples) {
								Game.GameGetSoundSamples(&Thread, &GameMemory, &SoundBuffer);
							}
