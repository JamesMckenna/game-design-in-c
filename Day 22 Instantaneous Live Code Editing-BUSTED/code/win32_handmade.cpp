...
int CALLBACK WinMain(
	...
	{ 	
		...
		CatStrings(OnePastLastSlash - EXEFileName, EXEFileName, sizeof(SourceGameCodeDLLFilename), SourceGameCodeDLLFilename, sizeof(SourceGameCodeDLLFullPath) - 1, SourceGameCodeDLLFullPath);
		
		char TempGameCodeDLLFilename[] = "handmade_temp.dll";
		char TempGameCodeDLLFullPath[MAX_PATH];
		CatStrings(OnePastLastSlash - EXEFileName, EXEFileName, sizeof(TempGameCodeDLLFilename), TempGameCodeDLLFilename, sizeof(TempGameCodeDLLFullPath) - 1, TempGameCodeDLLFullPath);
	...
	LPVOID BaseAddress = 0;
				...
				if(Samples && GameMemory.PermanentStorage && GameMemory.TransientStorage){
						...
						*NewKeyboardController = {};
						...
						if((GlobalSecondaryBuffer->GetCurrentPosition(&PlayCursor, &WriteCursor)) == DS_OK) { 
							...
							*/
							...
							OutputDebugStringA(TextBuffer);//GOOD UP TILL HERE
#endif
...
#if HANDMADE_INTERNAL
					{
						...
						if(SecondsElapsedForFrame < TargetSecondsPerFrame) {		
							if(SleepIsGranular) {
								DWORD SleppMS = (DWORD)(1000.0f * (TargetSecondsPerFrame - SecondsElapsedForFrame));
								if(SleepMS > 0){
									Sleep(SleepMS);
								}
							}
						...
						win32_window_dimension Dimension = Win32GetWindowDimension(TheWindow);
#if HANDMADE_INTERNAL
						Win32DebugSyncDisplay(&GlobalBackBuffer, ArrayCount(DebugTimeMarkers), DebugTimeMarkers->OutputPlayCursor, &SoundOutput, TargetSecondsPerFrame);
#endif
						Win32DisplayBufferInWindow(DeviceContext, Dimension.Width, Dimension.Height, &GlobalBackBuffer);
